...
for a lengthening glide; and **body + control-surface for the generic maneuvering body**, where the nose material barely
enters and the fin/airframe material *is* the answer. (Today's `heating.py` applies a **single**
`tps_material` everywhere — the per-location split is the §10.1 upgrade; the generic maneuvering body case is the sharpest
...
- **Glider** needs the **two range ceilings** — aerodynamic (can L/D, β reach the range?) and thermal
  ...
  ~1500 km).
- **Maneuvering (generic maneuvering body)** needs **(a) off-nose + AoA heating** (the hot spot is a fin LE / windward
...
modes** above. Per-location + AoA heating and the transient heat-sink are the only additions the generic maneuvering body
...
| **Non-separating, bare hot structure** (generic maneuvering body) | **the airframe material** (Al/steel/C-C/C-SiC) | **= `body_tps_material`** | **collapses** — surface limit *is* the structure limit |
...
- **`body_tps_thickness_m` reinterprets by group:** sacrificial → TPS-layer thickness (recession +
  ...
  (does the skin absorb the generic maneuvering body pull-up spike before exceeding its limit — §0).
- **Control surfaces/fins ride the body material for v1.** The generic maneuvering body hot spot (fin LE / windward) is
...
> time vs glide time; generic maneuvering body = fly the pull-up, the body location catches the spike). Bands,
...
- **Maneuver (generic maneuvering body):** sweep pull-up severity (g, altitude); find where a hot-spot margin → 1 → the
  **envelope**.
- **Verify:** reproduce this session's numbers — SWERVE ~100 s; AHW ~500 s @ 1,500 km, nose-limited @
  6,000 km.

### Phase 4 — generic maneuvering body specifics (the two additions §0 named)
**Files:** `heating.py`.
...
- **Verify:** a generic maneuvering body-class case binds on the fin/windward, not the nose; transient `T_w` < equilibrium.
