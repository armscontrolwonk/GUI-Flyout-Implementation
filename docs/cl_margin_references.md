...
- **Gulan, A. E., "Conceptual, Trajectory-Based Structural Sizing Method for
  ...
  **[verified]** — read against the thesis PDF 2026-09-08. A **second-hand
  source for the vehicle dimensions**; prefer the primaries below.
  **Table 2 (SWERVe, the public C-HGB predecessor):** length **2.75 m**,
  half-cone angle **5°**, span **0.87 m**, **4 fins**. Table 2 contains those
  four rows and nothing else.

  **Provenance of Table 2 (2026-09-08).** "SWERVe" here is not a separate
  vehicle: it is Sandia's SWERVE, reached through the Murbach / Aeolus
  Mars-derivative literature. Gulan's own citation for Table 2 is his ref [35] =
  Murbach, Keese & Farmer, SSC97-V-2, and the dimensions trace there — 2.75 m
  also in Murbach, AIAA 93-0313, p. 1 ("flown three times in the 2.75 m long
  version"); the 5° half-angle and the four cruciform wings in SSC97-V-2 p. 6
  ("a sharp 5 deg half-cone with four wings arranged in a cruciform"). Treat the
  row as a **downstream citation of Murbach** rather than independent
  corroboration. The **span of 0.87 m is the one entry not located in Murbach**;
  its ultimate source is unestablished.

  Two things to carry forward when using this row:

  * **The booster.** Gulan states it as an assumption — "It is assumed that this
    vehicle, like the C-HGB, will utilize the US Navy 0.876 meter booster." The
    flown article used Sandia's **STRYPI VIII-R** (Sandia LAB NEWS, 23 Jan 1981).
    The Navy association belongs to the AHW/CPS descendants: AvWeek (11 Oct 2018)
    reports the 2011 AHW flight on a Polaris-derived stack, and the Navy leads
    CPS.
  * **The 5° half-angle is a rounding.** The flown article is **5.25°**
  ...
  Derived base diameter ≈ 0.48 m sharp-cone (**superseded**: the flown vehicle is
  spherically blunted, giving 0.5385 m — see `ro_library/SWERVE.ro.json`).
  **Key modelling fact:** the **lift reference area is the fin area** (drag
  reference is the max cross-section) — i.e. the lift is generated by the four
  fins; they are primary lifting/control surfaces, not trim tabs.

  **NO FIN PLANFORM OR CHORD IS AVAILABLE HERE (corrected 2026-09-08).** An
  earlier note in this entry said the exact fin planform area / chord was "in
  the thesis Figure 24 / its ref [35], still to be pulled". Both pointers were
  checked against the PDF and neither holds: Table 2 (ref [35], Murbach) carries
  only the four rows above, and **Figure 24 is captioned "Representation of
  SWERVe" and cites ref [36] = Wang, B., "First US hypersonic weapon will be
  based on Old Sandia work," NextBigFuture.com** — an illustration, not a
  dimensioned drawing. Gulan needed a fin area for the same reason we do and
  obtained it from a parametric CAD model rather than from a citation.

  So the absence of a chord is structural across the whole chain — Iliff &
  Shafer, Murbach and Gulan alike — consistent with AvWeek (6 Aug 1990)
  recording that the vehicle's shape and performance were still classified. The
  open routes are to measure it from imagery (`image_measure.py`) or to generate
  the finned/deflected DATCOM deck (`TRIM_GATE_APPROACH.md` §7).
