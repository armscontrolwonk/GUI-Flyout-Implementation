...
|---|---|---|
| **Reentry-F** (Mach ~20 entry, ATJ graphite nosetip R_n 0.1 in; NASA CR-154044 / TM X-1856 / LWP-460, all via Berry's nose-tip white paper in the project Drive) | *tuning* — the in-envelope δ/R_n shape-change ladder, now with the radius-history spread quantified: R_n 0.10 → **0.17–0.30 in** best-supported (TM X-1856 curves 1–2 + pressure-derived estimates), 0.5 in worst case **refuted** by the report itself | the accuracy-band ladder plus a **derived H_eff bracket 50–135 MJ/kg, central ≈84** (Q ≈ 2.85 GJ/m² ±25%, digitized point-by-point from the clean nominal-trajectory figure supplied in-chat — `benchmarks/form_a/reentryf_nominal_qdot.csv` — ÷ 0.6–1.0 in axial-recession spread; peak read 324 MW/m² confirms the 318 MW/m² pin; nominal 40 over-predicts ~2× → conservative sign; Q_MJ stays None in code — preflight prediction, never flight-measured) |
...
| carbon_carbon | 1800 | 25 | **40** | 60 | C/C oxidation→sublimation regime ([OSTI carbon/graphite RV-nosetip correlation](https://www.osti.gov/biblio/4729765); table endpoints = engineering bracket). **Reentry-F flight-derived bracket 50–135 MJ/kg (central ≈84, digitized Q ≈ 2.85 GJ/m²)** — nominal 40 sits below it and over-predicts recession ~2×, the conservative side for a screen (`benchmarks/form_a/phase2-heff-bands.md` + `reentryf_nominal_qdot.csv`). **Validity floor:** Nestler 1979 (read from primary) shows the band does NOT apply at ≥80 atm stagnation pressure — severe-regime Q\* collapses to ~10–20 MJ/kg |
...
| severe blunting | δ/R_n = 0.50–1.0 | Berry, Reentry-F (NASA CR-154044): flew R_n 0.10→0.171 in (~0.7 R_n), survived; TM X-1856 (via Berry white paper) widens the survived-blunting spread to 0.17–0.30 in final radius (~0.7–2 R_n growth), worst-case 0.5 in refuted | ✔ cited (spread quantified) |
...
| Reentry-F H_eff bracket | 50–135 MJ/kg, central ≈84 (flight graphite, 5–60 atm) | derived: Q ≈ 2.85 GJ/m² ±25%, digitized point-by-point from the clean nominal-trajectory figure (γ_E 21.2°, V_E 20,300 ft/s; the figure Berry reproduces as his Fig. 6 [LWP-460]; reads + integration in `benchmarks/form_a/reentryf_nominal_qdot.csv`) ÷ (ρ 1.73 g/cc vendor-nominal × 0.6–1.0 in axial-recession spread, CR-154044 0.77 in central); peak read 324 MW/m² @ ~45–49 kft confirms the 318 MW/m² `_BENCHMARKS` pin; 100→50 kft window ~8 s; supersedes the earlier ~1 GJ/m² order-of-magnitude read (and the under-prediction caveat that rested on it); Q_MJ stays None in code — preflight-nominal, no flight-measured stagnation heating exists (TM X-2560) | ⚠ labeled derived (figure-digitized; nominal 40 over-predicts ~2×, conservative) |
