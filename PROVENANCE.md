...
| C-HGB total length (May) | 2.43 m | The image's labelled dimensions were mutually inconsistent. | 1.5 m, trusting the half-angles and the 0.58 m base diameter, per the maintainer's correction. The geometry was measured by the maintainer with Thrusty's mensuration tool (`image_measure.py`); the image and scale anchor were not recorded, because the tool's provenance stamp did not survive the save. | `ro_library/C-HGB.ro.json` |
